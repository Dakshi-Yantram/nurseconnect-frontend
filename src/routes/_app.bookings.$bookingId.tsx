...
function ConsumerBookingDetail() {
  ...
  const amount = domainBooking?.totalAmount != null
    ? Number(domainBooking.totalAmount) // backend stores rupees
  ...
  // app/services/cash_payment.py::is_cash_eligible), not guessed from
  // booking state here, so the rule lives in exactly one place and this
  // page can never drift from what the backend actually allows.
  useEffect(() => {
    let cancelled = false;
    if (!canPay) {
      setCashOption(null);
                ...
                <div className="mt-3 pt-3 border-t border-current/10">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* Cash option only rendered when the backend says this
                        ...
                        backend applies immediately with no frontend
                        release. */}
