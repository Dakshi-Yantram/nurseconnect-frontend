...
/**
 * Phase 8B — Consumer longitudinal care runtime.
 *
 * Surface is journey-oriented (Care alerts → In care now → Upcoming →
 * Recently completed → Care packages → Patients under care) rather than a
 * KPI dashboard. All bucketing comes from useConsumerCareSnapshot so this
 * route holds no derivation state, and every volatile region is wrapped
 * in RuntimeBoundary for hydration-safe degradation.
 */
...
function ConsumerLayout() {
  ...
  // Match both "/consumer" and "/consumer/" (trailing slash variants)
  if (pathname === "/consumer" || pathname === "/consumer/") return <ConsumerHome />;
  return <Outlet />;
}
...
function PackageLifecycleRow({
  ...
  // Lightweight lifecycle: derive a coarse progress signal from the
  // consumer's booking ledger that mentions this package's service tier.
  // Stays presentation-only — no new state.
